//this breaks emoji buttons (at the veyr least)
//when the order of resources in this array is 
//not identical is the id values currently in the constants
//oops
function getInitResources() {
  ...
  unicornRes,viperRes,goldenGrainRes,
  bloomRes,mandragoraRes,venomRes,goldDustRes,
  
  homonculiRes, baitDroneRes, 
  toxinDroneRes,guardDronesRes,zombiesRes, gemsRes
...
const bloom = { name: "Bloom", id: 24, emoji: `🌼` };
...
var familiarRes = new Resource(familiar, { activationDelay: 1 , vizRequirements:[{id:familiar,min:0}]},
  [new resDelta(TICK, mana, { a: .15, b: .39 }),
  new resDelta(`Upgrade`, mana, { a: -.245, b: -4.12 }),
...
//fairies
const a_Bait = `Bait`;
var fairyTrapRes = new Resource(fairyTraps, {vizRequirements:[{id:mana,min:40}]},
[ //trap fairies with mana
  new resDelta(TICK, fairies, { b: .02 }),
  new resDelta(TICK, fairyTraps, { b: -.005 }),
  new resDelta(a_Bait, mana, { a: -.05, b: -1, c: -45 }),
...
var unicornRes = new Resource(unicorns, {vizRequirements:[{id:fairyDust,min:25}]},
[ //summon pooping unicorns with fairy dust
  ...
  new resDelta(a_Summon, fairyDust, { a:-.01,b: -.3, c: -28 }),
...
//vipers
const a_Charm = `Charm`;
var viperRes = new Resource(vipers, { vizRequirements:[{id:bloom,min:3}]},
...
var mandragoraRes = new Resource(mandragora, { activationDelay: 20, vizRequirements: [{ id: magicBeans, min: 1 }, {id:fertilizer,min:1}] },
...
[
  new resDelta(TICK, mana, { a: .0001, b: .01 }),  
